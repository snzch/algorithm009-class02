...
class MyCircularDeque:
    ...
    def insertFront(self, value: int) -> bool:
        ...
        if self.size < self.max_size:
            self.head = self._pre_index(self.head)
    ...
    def deleteLast(self) -> bool:
        ...
        """
        if self.size:
            self.tail = self._pre_index(self.tail)
            self.size -= 1
            return True
        return False

    def _pre_index(self, index: int) -> int:
        return index - 1 if index > 0 else self.max_size - 1

    def _next_index(self, index: int) -> int:
        return index + 1 if index < self.max_size - 1 else 0
